...
def SigCsq_avg(params, lens, src):
    ...
    print('lens norm=', scipy.integrate.simpson(dNdzl, zl))
    print('source norm=', scipy.integrate.simpson(dNdzs, zs))
	
    Sigma_inv = sp.get_SigmaC_inv(params, zs, zl)
	
    Sig_inv_sq_zsint = [ scipy.integrate.simpson(Sigma_inv[:, zi]**2 * dNdzs, zs) for zi in range(len(zl))]
    Sig_inv_sq_avg = scipy.integrate.simpson(Sig_inv_sq_zsint * dNdzl, zl)
...
def cov_beta_Ups(params, lens, src, rp_bin_edges, rp0, Pimax, hder, endfilename):
	...
	int_in_k_gm = [[ scipy.integrate.simpson(k**2 * dPdbeta[mi] * invcov[mi] * dPdUps_gm[mi][rpi], k) for mi in range(len(mu))] for rpi in range(len(rp_bin_edges)-1)]
	int_in_k_gg = [[ scipy.integrate.simpson(k**2 * dPdbeta[mi] * invcov[mi] * dPdUps_gg[mi][rpi], k) for mi in range(len(mu))] for rpi in range(len(rp_bin_edges)-1)]
	
	# And in mu.
	print("Doing mu integration.")
	int_in_mu_gm = [ scipy.integrate.simpson(np.asarray(int_in_k_gm[rpi]), mu) for rpi in range(len(rp_bin_edges)-1)]
	int_in_mu_gg = [ scipy.integrate.simpson(np.asarray(int_in_k_gg[rpi]), mu) for rpi in range(len(rp_bin_edges)-1)]
