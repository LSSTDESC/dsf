...
def average_in_bins(F_, R_, Rp_):
    ...
    F_binned= [2. * scipy.integrate.simpson(F_f_[indlow[i]:indhigh[i]]* (R_f_[indlow[i]:indhigh[i]])**2, np.log(R_f_[indlow[i]:indhigh[i]])) / (R_f_[indhigh[i]]**2 - R_f_[indlow[i]]**2) for i in range(0, len(Rp_) - 1)]
...
def average_in_bins_2D(F_, R_, Rp_):
    ...
    F_binned_1 = [[2. * scipy.integrate.simpson(F_f_1[rpi, indlow[i]:indhigh[i]]* (R_f_[indlow[i]:indhigh[i]])**2, np.log(R_f_[indlow[i]:indhigh[i]])) / (R_f_[indhigh[i]]**2 - R_f_[indlow[i]]**2) for rpi in range(len(R_))] for i in range(0, len(Rp_) - 1)] 
    ...
    # Integrate in the first dimension	
    F_binned_2 = [[2. * scipy.integrate.simpson(F_f_2[indlow[i]:indhigh[i], rpi]* (R_f_[indlow[i]:indhigh[i]])**2, np.log(R_f_[indlow[i]:indhigh[i]])) / (R_f_[indhigh[i]]**2 - R_f_[indlow[i]]**2) for i in range(0, len(Rp_) - 1)] for rpi in range(len(Rp_)-1)]
...
def z_ofcom_func(params):
	...
	z_vec = np.linspace(0., 10., 4000) # This hardcodes that we don't care about anything over z=2100
