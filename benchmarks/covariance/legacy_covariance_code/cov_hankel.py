""" This script gets the covariance of Delta Sigma gm
...
def get_DeltaSigma_covs(params, rp_bin_edges, rp_bin_c, lens, src, endfilename):
    ...
    #HT=ht.hankel_transform(rmin=0.6,rmax=110,kmax=30.0,j_nu=[2],n_zeros=480000,kmin=10**(-4))
    
    # Get the required integrals over the line-of-sight window - these are all in units of Mpc/h
    ...
    ShapeNoise = sp.shape_noise(params, src, lens) # Units (Mpc/h)^2
    ShotNoise =  sp.shot_noise(lens) # Units (Mpc/h)^3
    vol = sp.volume(params, src, lens) # Volume of the lenses, units (Mpc/h)^3
	
    z, dNdzl = sp.get_dNdzL(params, lens)
    pk_ofz = [ccl.nonlin_matter_power(cosmo, k * params['h'], 1./ (1. + z[zi])) * params['h']**3 for zi in range(len(z))] # Units (MPc/h)^3
    pk_ofz_arr = np.zeros((len(k), len(z)))
    for zi in range(len(z)):
        pk_ofz_arr[:, zi] = pk_ofz[zi]
        
    pk = [ scipy.integrate.simpson(pk_ofz_arr[ki, :] * dNdzl, z) for ki in range(len(k)) ] # Units (MPc/h)^3
    
    save_pk = np.column_stack((k,pk))
    np.savetxt('./save_pk_legacy.dat',save_pk)
...
def get_DeltaSigma_covs_unbinned(params, rp_bin_edges, rp_bin_c, lens, src, endfilename):
    """ Get the total covariance matrix for joint probes.
    This version returns without averagin in rp bins.
    params : dictionary of cosmological parameters. 
    rp_bin_edges : edges of the projected radial bins 
    lens : label for the lens sample 
    src : label for the source sample """
    ...
    # Set up the hankel transform we need; we will use this for all the Delta Sigma covariances.
    k = np.logspace(np.log10(10**(-4)), np.log10(30.), 5000)
    HT=ht.hankel_transform(rmin=0.6,rmax=110,kmax=k[-1],j_nu=[2],n_zeros=100000,kmin=k[0])
    ...
    # Get the covariance of (DS_gm, DS_gm)
    (r, ht_gmgm) = get_cov_DeltSig_gm_unbinned(params, HT, k, pk, DPi2_gm, ShapeNoise, ShotNoise, vol, rp_bin_edges, lens, src)
    
    # Get the covariance of (DS_gg, DS_gg)
    (r, ht_gggg) = get_cov_DeltSig_gg_unbinned(params, HT, k, pk, DPi2_gg, ShotNoise, vol, rp_bin_edges)
    
    # Get the covariance of (DS_gm, DS_gg)
    (r, ht_gmgg) = get_cov_DeltSig_gmgg_unbinned(params, HT, k, pk, DPi2_gmgg, ShotNoise, vol, rp_bin_edges)
    
    return (r, ht_gmgm, ht_gggg, ht_gmgg)
...
def get_cov_DeltSig_gm(params, HT, k, pk, DPi2_l, ShapeNoise, ShotNoise, vol, rp_bin_edges, lens, src):
    ...
    p_g = np.asarray(pk) * params['b']**2 # Units (MPc/h)^3
    p_kappa = np.asarray(pk) * (rho_crit * (params['OmM']))**2 * DPi2_l # Units Msol^2 Mpc^2 / pc^4
    
    SigC2 = jp.SigCsq_avg(params, lens, src) # Units Msol^2 h^2 / pc^4

    # Taper and get the ggkk covariance term then bin in r, units Msol^2 Mpc^3 / (h pc^4)
    ...
    # Get the gkgk covariance term, units Msol^2 Mpc^3 / (h pc^4)
    ...
    cov = (cov_ggkk_bin + cov_gkgk_bin) / vol # Units Msol^2 h^2 / pc^4

    return (r_bin, cov)

def get_cov_DeltSig_gm_unbinned(params, HT, k, pk, DPi2_l, ShapeNoise, ShotNoise, vol, rp_bin_edges, lens, src):
    """ Get the covarariance of Delta Sigma gm at projected radius rp. 
    This version doesn't average in bins of rp before returning
    params : dictionary of cosmological parameters
    HT : Hankel Transform set up structure from Sukdheep's code
    k : k at which the matter power spectrum is calculated
    pk : matter power spectrum integrated over lens z dist
...
def get_cov_DeltSig_gg(params, HT, k, pk, DPi2, ShotNoise, vol, rp_bin_edges):
    ...
    #plt.figure()
    #plt.loglog(r_bin, np.diag(cov_gg_bin)/vol, 'o', label='gggg cov term')
    #plt.loglog(k, p_kappa + ShapeNoise , 'm', label='Pkk + shapenoise')
    #plt.legend()
    #plt.savefig('../plots/covgggg_DSgg_Aug2024.pdf')
    #plt.close()
...
def get_cov_DeltSig_gg_unbinned(params, HT, k, pk, DPi2, ShotNoise, vol, rp_bin_edges):
    """ Get the covarariance of Delta Sigma gg at projected radius rp.
    This version does not average in rp bins.
    params : dictionary of cosmological parameters
    HT : Hankel Transform set up structure from Sukdheep's code
    k : k at which the matter power spectrum is calculated
    pk : matter power spectrum integrated over lens z dist
...
def get_cov_DeltSig_gmgg(params, HT, k, pk, DPi2, ShotNoise, vol, rp_bin_edges):
    ...
    #plt.figure()
    #plt.loglog(r_bin, np.diag(cov_gmgg_bin)/vol, 'o', label='gkgg cov term')
    #plt.loglog(k, p_kappa + ShapeNoise , 'm', label='Pkk + shapenoise')
    #plt.legend()
    #plt.savefig('../plots/covgmgg_DSgmgg_Aug2024.pdf')
    #plt.close()
...
def get_cov_DeltSig_gmgg_unbinned(params, HT, k, pk, DPi2, ShotNoise, vol, rp_bin_edges):
    """ Get the covarariance of Delta Sigma gm with Delta Sigma gg at 
    projected radius rp. 
    This version doesn't bin in rp.
    params : dictionary of cosmological parameters
    HT : Hankel Transform set up structure from Sukdheep's code
    k : k at which the matter power spectrum is calculated
    pk : matter power spectrum integrated over lens z dist
...
def W_ofPi_lensing(params, lens, src):
    ...
    print('SigCavg=', SigCavg)
	
    (zl, dNdzl) = sp.get_dNdzL(params,lens)
    chil = ccl.background.comoving_radial_distance(cosmo_fid, 1./(1.+zl)) * params['h'] # Mpc/h
    ...
    Pipos = np.logspace(np.log10(0.1), np.log10(3000),4000)
    Pi_rev= list(Pipos)
    Pi_rev.reverse()
    Pi_neg = Pi_rev 
    ...
    z_Pi = [[(1./ccl.scale_factor_of_chi(cosmo_fid, chil[zli] + Pi[pi]) - 1.) if (chil[zli] + Pi[pi]>=0.) else -100. for pi in range(len(Pi))] for zli in range(len(zl))]
	
    #print("zPi=", z_Pi[2000])

	# Get the radial comoving distance at the source z's and at z_Pi (lenses)	
    com_s = ccl.background.comoving_radial_distance(cosmo_fid, 1./(1.+zs)) * params['h'] # Mpc/h
    com_zPi = [[chil[zli] + np.asarray(Pi[pi]) for pi in range(len(Pi))] for zli in range(len(zl))] # Mpc/h

    # Get Sigma_crit(chis, chil + Pi)
    Sig_inv_ofPi = [ [ sp.get_SigmaC_inv_com(params, com_s, com_zPi[zli][pi], z_Pi[zli][pi]) for zli in range(len(zl))]  for pi in range(len(Pi)) ]  # Units pc^2 / (Msol h)

    print('Sig inv of Pi=', Sig_inv_ofPi[2000][250])

    # Integrate this over source redshift distribution
    zs_int = [[ scipy.integrate.simpson(np.asarray(Sig_inv_ofPi[pi][zli]) * dNdzs, zs) for zli in range(len(zl))] for pi in range(len(Pi))] # Units pc^2 / (Msol h)
    
    print('zs_int=', zs_int[2000])

    # Integrate then over lens distribution
    zl_int = [ scipy.integrate.simpson(zs_int[pi] * dNdzl, zl) for pi in range(len(Pi))] # Units pc^2 / (Msol h)
	
    print('zl_int=', zl_int[2000][250])

	# Mutliply by the fully averaged SigC over the full lens and source
    W_ofPi = np.asarray(zl_int) * SigCavg # Dimensionless
...
def DeltaPi_gm(params, lens, src):
	...
	(Pi, W) = W_ofPi_lensing(params, lens, src) # W is dimensionless
	
	DeltaPi1 = scipy.integrate.simpson(W, Pi) # I guess then this output is in Mpc/h
	
	DeltaPi2 = scipy.integrate.simpson(W**2, Pi) # and this one too
...
def DeltaPi_gg(params, lens):
    ...
    if lens=='LOWZ':
        ...
        DeltaPi1 = (ccl.comoving_radial_distance(cosmo, 1./ (1. +1.0))* params['h'] ) - (ccl.comoving_radial_distance(cosmo, 1./ (1. +0.4))* params['h'] )
        DeltaPi2 = DeltaPi1 
    elif (lens=='DESI_plus_20pc' or lens=='DESI_plus_50pc' or lens=='DESI_4MOST_LRGs' or lens=='DESI150pc_4MOST_LRGs' or lens=='DESI200pc_4MOST_LRGs'):
...
def DeltaPi_gmgg(params, lens, src):
    ...
    if lens=='LOWZ':
        ...
        chi_diff_low = ccl.comoving_radial_distance(cosmo, 1./ (1. +0.4))* params['h']  - ccl.comoving_radial_distance(cosmo, 1./ (1. +0.72))* params['h'] 
        chi_diff_high = ccl.comoving_radial_distance(cosmo, 1./ (1. +1.0))* params['h']  - ccl.comoving_radial_distance(cosmo, 1./ (1. +0.72))* params['h']
    ...
    # Save the Pi grid:
    np.savetxt('./Pi_grid.dat', Pi_integ)
	
    DeltaPi2 = scipy.integrate.simpson(W_integ, Pi_integ)
