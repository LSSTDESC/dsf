...
def get_dNdzL(params, lens_sample):
    ...
    if (lens_sample=='DESI'):
        ...
        norm = scipy.integrate.simpson(dNdz_getnorm, z_highres)
        ...
        return (z_highres, dNdz_getnorm / norm)
    elif (lens_sample=='DESI_LRG_dp2-ggl'):
        dNdz_file = './dNdz_lens_DESILRG_1bin.dat'
...
def get_dNdzS(src_sample):
    ...
    if (src_sample == 'LSST'):
        ...
        norm = scipy.integrate.simpson(nofz_, z)
    ...
    elif (src_sample == 'LSSTY1_dp2-ggl'):

        # This case already includes the photo-z uncertainty
        # no need to incorporate directly.

        dNdz_file = './dNdz_source_LSSTY1Bin5.dat'
        z, dNdz = np.loadtxt(dNdz_file, unpack=True)
		
        interpolation = scipy.interpolate.interp1d(z, dNdz)
	
        # Create a well-sampled redshift vector to make sure we can get the normalization without numerical problems
        z_highres = np.linspace(z[0], z[-1], 500)
...
def dNdz_perturbed(z, dNdz, sigma, deltaz):
    ...
    if (np.abs(sigma)>10**(-12)):
        ...
        denominator = scipy.integrate.simpson(numerator, z_new)
...
def shape_noise(params, src, lens):
    ...
    The units for ns are (number of galaxies / (Mpc/h)^2) - this does not include the factor of area here.
    src: source sample. """
    ...
    if ((src=='LSST' and lens=='DESI')):
        ...
        # 46656000 is 4 * 180**2 * 3600. 4 pi * (180/pi)**2 is the number of square degrees is a sphere.
        n_s = n_s_amin * (466560000. / np.pi) / (4 * np.pi * chieff**2) # This is in galaxies / (Mpc/h)^2.
        shape_noise = sig_gam**2 / n_s # So this is in (Mpc/h)^2/galaxies or just (Mpc/h)^2
        return shape_noise
        ...
        #return shape_noise
    elif (src=='LSSTY1_dp2-ggl' and lens=='DESI_LRG_dp2-ggl'):
        # This is for the 5th source bin *only*, creating benchmark for DP2 project comparison.
        sig_gam = 0.26
        n_s_amin = 2.025 # Fifth source bin only, copied over from dsf output.
        chieff = 1788.0146317375684 # Mpc/h, copied directly from dsf output.
        #chieff = ccl.comoving_radial_distance(cosmo_fid, 1./ (1.+0.72))* params['h'] # z=0.72 is mean redshift of DESI LRG lenses
        #print('chieff=', chieff)
        # 46656000 is 4 * 180**2 * 3600. 4 pi * (180/pi)**2 is the number of square degrees is a sphere.
        n_s = n_s_amin * (466560000. / np.pi) / (4 * np.pi * chieff**2) # This is in galaxies / (Mpc/h)^2.
        print((466560000. / np.pi) / (4 * np.pi))
        shape_noise = sig_gam**2 / n_s # So this is in (Mpc/h)^2/galaxies or just (Mpc/h)^2
        return shape_noise
...
def shot_noise(lens):
	...
	if (lens=='DESI' or lens=='DESI_LRG_dp2-ggl'):
...
def volume(params, src, lens):
    ...
    if ((src=='LSST' or src == 'LSSTY10' or src =='LSSTY1_dp2-ggl') and (lens=='DESI' or lens=='DESI_LRG_dp2-ggl')):
